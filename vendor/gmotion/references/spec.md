...
```jsonc
{
  ...
  "depth":   0.34,                       // 배경이 카메라를 따라가는 비율. false 면 깊이 없음
  "shutter": true,                       // 전환의 모션블러. false 면 끔. 숫자면 세기 배율
...
**기본값이 전부 켜짐이다.** 적지 않아도 동작하고, 끄고 싶을 때만 적는다.

| 필드 | 무엇 | 끄면 |
|---|---|---|
| `camera` | 씬마다 카메라가 **씬 전체 길이(hold 포함)** 동안 아주 느리게 움직인다 | hold 가 정지 프레임이 된다 — 슬라이드로 읽힌다 |
| `depth` | 배경 레이어가 카메라의 `depth` 배만 따라간다(기본 `0.34`) | 배경과 피사체가 같이 움직여 "사진 확대"로 보인다 |
| `shutter` | 움직임이 있는 전환(`cut` `push*` `zoom*` `match` 등)에 짧은 모션블러가 붙는다 | 빠른 컷이 순간이동처럼 보인다 |

세 값 모두 `false`(끔) · `true`(기본값) · 숫자(배율)를 받는다. `depth` 는 0~1 이고
**0.2~0.45 가 적정**이다 — 1 에 가까우면 배경이 피사체를 그대로 따라가 깊이가 사라진다.
`camera: 0.5` 는 진폭을 절반으로, `shutter: 1.6` 은 잔상을 강하게 한다.
진폭·잔상은 `energy` 가 이미 조절한다(E1 은 카메라를 더 움직이고, E3 은 덜 움직이는 대신
잔상이 강하다) — 배율은 그 위에 곱해진다.


## design — 이 스펙에서만 쓰는 커스텀 요소

기본 요소(테마 15 · 스킨 6 · 픽토그램 238 · 벡터 79)로 안 되면 여기에 정의한다.
...
| 필드 | 뜻 |
|---|---|
| `pattern` | **필수.** 28종 중 하나 (`gm info patterns`) |
...
| `cam` | **이 씬의 카메라.** `pushIn` `pullOut` `panLeft` `panRight` `tiltUp` `tiltDown` `none`. 생략하면 패턴에 맞는 기본값 |
...
**씬 카메라 7종.** 씬 전체 길이(hold 포함)를 덮는 아주 느린 움직임 하나가 걸린다.
**정지 프레임을 없애는 것이 목적**이므로 진폭은 눈에 "움직임"으로 보이지 않을 만큼
작다 — 보이기 시작하면 산만해진다.

| 값 | 무엇 | 어디에 |
|---|---|---|
| `pushIn` | 천천히 다가간다 | 설명·나열·축적 (**대부분의 기본값**) |
| `pullOut` | 살짝 크게 시작해 안착한다 | 선언·인용·클로징 (`heroReveal` `kineticType` `quote` `endCard` `matchCut` 의 기본값) |
| `panLeft` `panRight` | 좌우로 흐른다 | 나열·목록·장 넘김 (`chapterCard` 는 `panRight`) |
| `tiltUp` `tiltDown` | 위아래로 흐른다 | 층·순위·아래로 내려가는 구조 |
| `none` | 카메라를 세운다 | 자체 움직임이 이미 있는 씬 (`marquee` `orbit` 의 기본값) |

**`zoomDetail` 과 `cameraJourney` 에는 적지 않는다.** 두 패턴은 카메라를 직접 쓰므로
씬 카메라를 얹지 않고, 적으면 validate 가 무시된다고 짚는다.

한 씬만 세우고 싶으면 `"cam": "none"`, 영상 전체를 세우려면 루트 `"camera": false` 다.
...
## chapterCard — 챕터 카드
유튜브 영상의 장 구분. 큰 번호가 서고 레일이 **"전체 중 지금 여기"** 를 말해 준다.

```jsonc
{ "pattern": "chapterCard",
  "no": 2,                               // 숫자면 "02" 로 자리수를 채운다. "PART 3" 처럼 글자면 그대로
  "title": "값은 어디서 생기나",
  "sub": "비용 구조를 뜯어 본다",
  "chapters": [                          // 전체 장 이름. 6개까지
    "무엇이 문제인가", "값은 어디서 생기나", "무엇을 바꿀 것인가"
  ],
  "current": 2 }                         // 1부터. 생략하면 숫자 no 를 쓴다
```

`chapters` 대신 `of`(전체 장 수)만 주면 이름 없이 칸만 그린다. 장 이름은 **칸 폭에 다
들어갈 때만 전부** 적고, 넘치면 지금 장 이름만 레일 아래 가운데에 적는다.
**아이콘·일러스트 자리는 없다** — 큰 번호가 이 씬의 시각 앵커라 그림을 더하면 서로 경쟁한다.

## rankList — 랭킹
Top N 순위. **1위부터 순서대로 적는다** — 자리는 순위가 정하고 `order` 는 공개 순서만 뒤집는다.

```jsonc
{ "pattern": "rankList",
  "title": "많이 쓰는 순서",
  "unit": "%",                           // 공통 단위 (항목의 unit 이 이긴다)
  "order": "countdown",                  // 기본. 낮은 순위부터 열려 아래에서 위로 채워진다 · "up" 은 적은 순서대로
  "items": [                             // 필수. 6개까지. 1위부터
    { "label": "검색", "value": 41, "icon": "search", "note": "가장 먼저 열린다" },
    { "label": "책갈피", "value": 27, "icon": "book" },
    { "label": "링크 공유", "value": 18, "icon": "link" }
  ],
  "top": true }                          // 1위 강조. 기본 true
```

`rank` 를 생략하면 배열 순서(1부터)가 순위다. 1위 링은 **표가 다 찬 뒤에** 감기고
E3 에서는 임팩트가 붙는다 — 결론은 마지막에.

## quizReveal — 퀴즈
질문 → 선택지 → 정답. **선택지 뒤의 정지(`beat`)가 이 패턴의 핵이다** — 답을 바로 열면
시청자가 스스로 꺼내지 않는다.

```jsonc
{ "pattern": "quizReveal",
  "kicker": "잠깐 멈춰 보자",
  "question": "이 셋 중 비용이 가장 큰 것은?",   // 필수(없으면 title). 질문이 곧 타이틀이라 mark·textFx 를 그대로 받는다
  "options": [                           // 4개까지. A·B·C·D 키가 자동으로 붙는다
    { "label": "찾는 시간", "correct": true, "icon": "search" },
    { "label": "저장 용량", "icon": "database" },
    { "label": "결재 대기", "icon": "clock" }
  ],
  "beat": 1.6,                           // 선택지 뒤 생각할 틈, 초. 기본 1.2. 에너지 배율을 따른다
  "answer": { "label": "찾는 시간이 나머지 둘을 만든다",   // 정답 선택지와 같은 글자면 소용없다 — 링이 이미 가리킨다
               "note": "저장도 결재도 못 찾아서 늘어난다" },
  "reveal": true }                       // 기본 true. false 면 답을 열지 않는다(다음 씬에서 답할 때)
```

공개는 정답 옵션에 good 색 링이 감기고 나머지가 흐려지며(opacity .34) 정답 띠(체크
아이콘)가 열리는 순서다. 씬 길이 상한은 **15초** — 정지까지 포함한 값이다.

## endCard — 엔드카드
영상의 마지막 씬. 구독·알림 청하기와 다음 영상 권하기를 한 화면에서 끝낸다.

```jsonc
{ "pattern": "endCard",
  "title": "다음 편에서 계속",
  "sub": "매주 한 편",
  "cta": [                               // 생략하면 구독(user)·좋아요(thumbup)·알림(bell) 셋이 기본
    { "label": "구독", "icon": "user" },
    { "label": "알림", "icon": "bell" }
  ],
  "next": [                              // 다음 볼 것. 2개까지. 카드 규칙은 cardsCascade 와 같다
    { "label": "비용 구조 편", "note": "이 편의 전제", "icon": "chart", "badge": "1편" },
    { "label": "도구 비교 편", "note": "다음 편", "art": "balance" }
  ],
  "handle": "@채널이름" }                 // 화면 맨 아래 한 줄
```

CTA 는 알약 모양 칩으로 서고 **마지막에 한 번 맥동한다** — 이 화면에서 시청자가 할 일이
그것이다. `next` 는 2개까지다(밀도 상한 2) — 셋을 권하면 아무것도 안 고른다.
