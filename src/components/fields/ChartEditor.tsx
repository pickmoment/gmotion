...
const OPTS: Record<
  string,
  { key: string; label: string; type: "num" | "str" | "bool" | "tone" }[]
> = {
  _all: [
  ...
  bar: [
    { key: "emphasis", label: "강조할 인덱스", type: "num" },
    { key: "labels", label: "값 라벨", type: "bool" },
  ],
  barH: [
    { key: "emphasis", label: "강조할 인덱스", type: "num" },
    { key: "sort", label: "큰 값부터 정렬", type: "bool" },
  ],
  ...
  bullet: [
    { key: "max", label: "축 상한", type: "num" },
    { key: "sharedScale", label: "모든 행이 같은 축", type: "bool" },
  ],
  barStack: [{ key: "horizontal", label: "가로로 눕힘", type: "bool" }],
  isotype: [
    { key: "icon", label: "칸 픽토그램", type: "str" },
    { key: "cols", label: "한 줄 칸 수", type: "num" },
  ],
          ...
          <div className="field" key={f.key}>
            ...
            {f.type === "bool" ? (
              <input
                type="checkbox"
                checked={!!o[f.key]}
                onChange={(e) => setOpt(f.key, e.target.checked || undefined)}
              />
            ) : f.type === "tone" ? (
              <select
      ...
      {chart === "waterfall" && (
        <div className="field">
          ...
          <input
            ...
            placeholder="0, 4"
            onChange={(e) =>
              setOpt("totals", nums(e.target.value).length ? nums(e.target.value) : undefined)
            }
          />
        </div>
      )}
        ...
        return (
          <div className="tr" key={i}>
            ...
            <input
              ...
              onChange={(e) =>
                patch(i, "value", e.target.value === "" ? undefined : Number(e.target.value))
              }
            />
            {bullet && (
              <input
                type="number"
                step="any"
                value={o.target === undefined ? "" : String(o.target)}
                onChange={(e) =>
                  patch(i, "target", e.target.value === "" ? undefined : Number(e.target.value))
                }
              />
            )}
            <button
...
function SeriesTable({
  ...
  return (
    <div className="table">
      ...
      {series.length > 8 && (
        <p className="warn-inline">색은 순환하지 않는다 — 9번째 시리즈는 만들 수 없다</p>
      )}
    </div>
  );
}
        ...
        <div className="tr" key={i}>
          {(["x", "y", "size"] as const).map((k) => (
            <input
              ...
              type="number"
              step="any"
              value={p[k] === undefined ? "" : String(p[k])}
              onChange={(e) =>
                patch(i, k, e.target.value === "" ? undefined : Number(e.target.value))
              }
            />
          ))}
          <input
