...
export type ItemFieldKey =
  | "label" | "text" | "when" | "icon" | "note" | "value" | "unit" | "prefix" | "dec"
  | "tone" | "badge" | "ribbon" | "art" | "spark" | "say" | "hub" | "ring"
  | "emphasis" | "scale";
...
export type Field =
  ...
  | { k: "select"; key: string; label: string; hint?: string; opts: () => Record<string, string>; req?: boolean }
  | { k: "icon"; key: string; label: string; hint?: string }
  | { k: "items"; key: string; label: string; hint?: string; req?: boolean; primary: ItemFieldKey; fields: ItemFieldKey[] }
...
const opt = {
  ...
  cardDir: () => ({ up: "up — 아래에서(기본)", left: "left — 왼쪽에서", scale: "scale — 확대", stack: "stack — 겹쳐 있다 흩어짐" }),
};
...
export const PATTERNS: Record<string, PatternSchema> = {
  heroReveal: {
    ...
    max: null,
    fields: [
      { k: "multiline", key: "title", label: "제목", req: true, hint: "줄바꿈하면 줄 단위 마스크 리빌이 걸린다", rows: 3 },
  ...
  networkBuild: {
    ...
    fields: [
      ...
      { k: "strings", key: "links", label: "연결", ph: 'API Gateway>인증  (또는 0>2)', hint: "생략하면 hub 에 전부 연결한다" },
  ...
  zoomDetail: {
    ...
    fields: [
      ...
      {
        k: "group", key: "detail", label: "상세 패널", hint: "확대된 뒤 옆에 붙는다",
        fields: [
          { k: "text", key: "title", label: "제목" },
  ...
  matchCut: {
    ...
    fields: [
      ...
      { k: "icon", key: "anchorTo", label: "모프 대상", hint: "주면 앵커가 그 도형으로 변형된다(MorphSVG)" },
      {
        k: "group", key: "from", label: "이전", fields: [
          { k: "multiline", key: "title", label: "제목", rows: 2 },
          { k: "text", key: "sub", label: "서브" },
        ],
      },
      {
        k: "group", key: "to", label: "이후", req: true, fields: [
  ...
  deviceShow: {
    ...
    fields: [
      ...
      {
        k: "group", key: "screen", label: "화면 안", hint: "줄·항목은 7개까지. 넘으면 화면이 아니라 문서가 된다",
        fields: [
          { k: "text", key: "title", label: "화면 제목" },
          { k: "strings", key: "lines", label: "줄", ph: "terminal 은 $ 로 시작하면 명령으로 본다" },
          items("items", "항목", {}),
...
export const COMMON_FIELDS: Field[] = [
  ...
  { k: "text", key: "purpose", label: "용도(purpose)", hint: "쓰다 막히면 그 씬은 필요 없는 씬이다" },
  { k: "multiline", key: "notes", label: "발표자 노트", rows: 3, hint: "--present 산출물에만 실린다" },
  { k: "multiline", key: "say", label: "대사(say)", rows: 3, hint: "--subs 로 빌드할 때 자막에서 이 글자를 찾아 씬 시작·길이를 실측으로 정한다" },
  { k: "number", key: "hold", label: "머무는 시간(초)", hint: "생략하면 글자 수로 추정. 자막에 맞추면 무시된다", step: 0.1 },
  ...
  { k: "select", key: "textFx", label: "글자 효과", opts: () => ({ scramble: "scramble — 섞이다 정렬", roll: "roll — 굴러 교체(matchCut 전용)" }) },
  { k: "strings", key: "decor", label: "배경 레이어", ph: "blob / grid …  한 줄에 하나(겹친다)", hint: "생략하면 루트 → 테마 기본" },
  { k: "select", key: "decorLevel", label: "배경 세기", opts: () => ({ "0": "0 — 약", "1": "1 — 기본", "2": "2 — 강" }) },
];
...
export function blankScene(pattern: string): Record<string, unknown> {
  ...
  switch (pattern) {
    case "heroReveal": s.title = "제목"; break;
    case "kineticType": s.lines = ["첫 줄", { text: "강조할 줄", emphasis: true }]; break;
    case "cardsCascade": s.title = "제목"; s.items = ["항목 1", "항목 2", "항목 3"]; break;
    case "networkBuild": s.title = "제목"; s.nodes = [{ label: "중심", hub: true }, { label: "노드 1" }, { label: "노드 2" }]; break;
    case "processFlow": s.title = "제목"; s.steps = ["1단계", "2단계", "3단계"]; break;
    case "beforeAfter":
      s.title = "제목";
      s.before = { label: "BEFORE", items: ["지금은 이렇다"] };
      s.after = { label: "AFTER", items: ["이렇게 바뀐다"] };
      break;
    case "explodedDiagram": s.title = "제목"; s.layers = ["층 1", "층 2", "층 3"]; break;
    case "zoomDetail":
      s.title = "제목"; s.items = ["항목 1", "항목 2", "항목 3"]; s.focus = 1;
      s.detail = { title: "상세", points: ["포인트 1"] };
      break;
    case "dataCounter": s.title = "제목"; s.stats = [{ value: 41, unit: "%", label: "지표" }]; break;
    case "timeline": s.title = "제목"; s.events = [{ when: "1월", label: "사건" }]; break;
    case "splitCompare":
      s.title = "제목";
      s.left = { label: "왼쪽", items: ["항목"] };
      s.right = { label: "오른쪽", items: ["항목"] };
      break;
    case "convergence": s.title = "제목"; s.sources = ["원천 1", "원천 2", "원천 3"]; s.target = { label: "도착점" }; break;
    case "divergence": s.title = "제목"; s.source = { label: "출발점" }; s.targets = ["도착 1", "도착 2", "도착 3"]; break;
    case "orbit": s.title = "제목"; s.center = { label: "중심" }; s.orbits = ["위성 1", "위성 2", "위성 3"]; break;
    case "matchCut": s.anchor = "question"; s.from = { title: "이전" }; s.to = { title: "이후" }; break;
    case "cameraJourney": s.title = "제목"; s.stops = ["정류장 1", "정류장 2", "정류장 3"]; break;
    case "marquee": s.title = "제목"; s.items = ["항목 1", "항목 2", "항목 3", "항목 4"]; break;
    case "chart": s.chart = "bar"; s.title = "제목"; s.data = { items: [{ label: "1월", value: 12 }, { label: "2월", value: 18 }] }; break;
    case "deviceShow": s.title = "제목"; s.frame = "browser"; s.screen = { title: "화면", items: ["항목 1"] }; break;
    case "quote": s.text = "인용문"; break;
  }
  return s;
}
