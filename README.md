...
| 픽토그램         | 238종을 한글 별칭 870개로 검색해 고른다 — 이름을 외워 쓰지 않는다                                                                                                                                                                                                                                                                                            |
| 차트             | 17종. 차트 종류에서 데이터 형태(단일·다중·heatmap·scatter)를 끌어내 맞는 표를 그린다                                                                                                                                                                                                                                                                         |
| 미리보기         | 실제 산출물을 iframe 에. 씬을 바꾸면 그 씬을 **처음부터 재생하고 씬 끝에서 멈춘다**. 타임라인 스크러버·현재 시각·씬 경계 틱·프레임 스텝·배속. iframe 은 **동일 출처가 아니다** — 조작은 런타임의 postMessage 다리로 간다(아래 절)                                                                                                                            |
| 검증             | `gm validate` 와 같은 오류(✗)·경고(!). 경고는 연출에 대한 지적이라 숨기지 않는다. **씬에 대한 지적은 눌러 그 씬으로 가고**, 씬 목록에 ✗·! 개수가 배지로 붙는다                                                                                                                                                                                               |
...
| 자막 → 스펙 초안 | 로컬 에이전트 CLI(claude·codex·pi·omp)로 **씬 표 → 스펙 JSON** 두 단계를 거쳐 받고, **앱의 검증기로 채점해 되먹인다**. 프롬프트는 임시 폴더의 `prompt.md` 로 써서 stdin(claude·codex) 또는 `@파일`(pi·omp)로 넘긴다 — argv 는 Windows 길이 한계에 걸린다. 모델은 CLI 별로 골라 넣을 수 있다(비우면 CLI 기본값). CLI 가 없으면 자막을 그대로 배치한 뼈대 초안 |
| 내보내기         | HTML(일반·발표용·클린) · 접근성 대본 HTML · 타임코드 CSV · MP4. 무엇이 실렸는지(자막·화면자막·음성) 결과에 명시한다                                                                                                                                                                                                                                          |
| MP4 렌더         | 아래 절                                                                                                                                                                                                                                                                                                                                                      |
| 검수             | `gm check` 정책 검사 + 전체 씬 contact sheet. 전체/감소 모션, CC, 일반·쇼츠·자막 안전 영역을 같은 산출물로 비교한다                                                                                                                                                                                                                                          |
| 접근성           | OS 감소 모션 대응과 강제 미리보기 · 플래시 빈도 제한 · 역할별 색 대비 · 자막 속도/겹침/길이 진단 · 장면 설명/화면 글자/내레이션/캡션 대본 내보내기                                                                                                                                                                                                           |
...
미리보기 트랜스포트: 스크러버(씬 경계 틱) · **씬 다시**(이 씬을 처음부터) · 재생/정지(씬 경계를 넘어 계속) ·
`‹` `›` 한 프레임 · 전체(0초부터) · 배속(0.5·1·1.5·2×, 음성이 있으면 음성도 같이) · `CC`(자막을 얹었을 때만).
미리보기에 포커스가 없어도 `Space` 재생/정지 · `←` `→` 씬 이동 · `,` `.` 한 프레임이 먹는다(글자를 치는 중에는 비켜 준다).
...
멈추거나 ffmpeg 이 실패하면 만들다 만 파일도 지운다(실패 문구에 ffmpeg 오류 꼬리를 붙인다).
...
움직이지 않는다(런타임이 `ready` 를 보내지 않아 씬 시킹·전송부도 죽는다). 산출물마다 스크립트가
달라 nonce·해시로는 좁힐 수 없다.

**그 iframe 은 `allow-same-origin` 없이 띄운다.** 스펙의 `svg`·마크·일러스트는 에이전트 CLI
출력이나 남이 준 파일에서 오므로 신뢰할 수 없고, 동일 출처면 그 안의 `<svg onload=…>` 가
앱 문서에서 돌아 `__TAURI_INTERNALS__.invoke('write_text'|'remove_file', …)` 까지 닿는다.
그래서 씬 시킹·재생은 런타임의 postMessage 다리로 한다 —
부모→산출물 `{gg:"cmd", op:"seek|goto|play|pause|replay|step|rate|captions"}`,
산출물→부모 `{gg:"ready"}` · `{gg:"state", t, playing}`(`vendor/gmotion/assets/runtime.js` 의 `bridge()`).
앱 문서 안에 그리는 SVG(디자인 스튜디오 미리보기)는 `src/lib/svg.ts` 로 걸러 넣는다.
