...
## 36. 회화 계열 테마 여섯 + 배경 여섯 · 배경 id 충돌 수정 (2026-09-05)

**왜.** 테마 15종은 전부 "용도"로 갈라져 있었다(리포트·B2B·헬스케어…). 화면의 인상을
정하는 축 — **회화의 문법**(붓질·문양·색면·오림·망점) — 이 비어 있어서, 문화·예술·캠페인
쪽 내용을 만들 때 고를 것이 `plum` 하나였다. 색만 바꾼 팔레트를 더 넣는 것은 의미가
없으므로 **색 + 기본 배경 + 폰트 + 기본 스킨**을 한 벌로 묶었다.

키는 **사조·기법 이름**이다. 특정 화가의 이름을 키로 쓰면 "그 화가의 그림처럼 보인다"는
약속이 되어 버린다 — 계보는 라벨에만 적는다.

### `assets/gsapgraph.js` — 테마 15 → 21종
| 키 | 색 | 폰트 | 기본 배경 | 기본 스킨 |
|---|---|---|---|---|
| `impasto` | 남색 야경 + 크롬 옐로 (다크) | serif | `swirl`+`constellation` | glass |
| `ukiyo` | 화지 + 프러시안 블루·인주 (라이트) | classic | `seigaiha`+`creases` | paper |
| `destijl` | 오프화이트 + 삼원색 (라이트) | gothic | `blocks`+`grid` | brutalist |
| `colorfield` | 짙은 적갈색 색면 (다크) | serif | `fields` | flat |
| `fauve` | 크림 + 코랄·터콰이즈 (라이트) | round | `cutouts`+`dots` | flat |
| `popart` | 노란 바탕 + 마젠타·코발트 (라이트) | impact | `halftone` | brutalist |

대비는 여섯 다 **AA 통과**다(`gm test` 의 역할별 최소 대비). 두 색은 기준에 걸려 고쳤다 —
`fauve.accent` 4.42 → 4.91(#cf3b25 → #c4351f), `colorfield.accent2` 4.54 → 5.45.
라이트가 여섯에서 **열**이 되어 밝은 발표장·인쇄 쪽 선택지가 늘었다.

### `assets/vectors.js` — 배경 20 → 26종
`swirl`(감기는 붓 자국 + 짧은 터치, 살짝 번지게) · `seigaiha`(반원 세 겹의 비늘, **아래쪽
에만 깔고 위로 사라진다**) · `blocks`(비대칭 분할 + 원색 면) · `fields`(번진 색면 둘) ·
`cutouts`(오린 잎 조각) · `halftone`(망점, **중심을 비우는 방사 페이드**).

눈으로 보고 세 번 고쳤다. 문양을 화면 전체에 균일하게 깔면 배경이 아니라 **벽지**가 되고
글자가 그 위에서 떨린다 — seigaiha 는 하단 밴드로, halftone 은 가장자리로 물러나게 했고,
swirl 은 또렷한 선(그림 기호처럼 읽혔다)을 굵게·크게·번지게 바꿨다.

### `assets/gsapgraph.js` — 배경 id 를 씬마다 가른다 (잠복 버그)
- **증상.** 새 배경을 붙이니 **1번 씬에만** 배경이 보였다. 배경은 씬마다 다시 그려지는데
  `defs` 의 id 는 고정 문자열이라(`ggSeigaiha`·`ggGridSmall`…) 한 문서에 같은 id 가 여러 번
  실린다. 브라우저는 첫 번째만 쓰고, **패턴·마스크는 다른 `<svg>` 뿌리에 있으면 아예
  칠해지지 않는다.** 기존 `gridPaper`(blueprint 테마 기본)도 같은 상태였다.
- **고침.** `scopeSvgIds(svg, tag)` 를 두고 배경을 그릴 때 `id="gg…"` · `url(#gg…)` ·
  `href="#gg…"` 를 `씬번호_레이어번호` 로 갈라 준다. 번호가 순서에서 나오므로 결정적이다 —
  CLI 와 앱의 산출물이 여전히 바이트까지 같다.

### 문서
`references/theming.md`(테마 표 + 회화 계열 절 · 기본 배경 표 6행 · 배경 26종 목록) ·
`MANUAL.md`(테마 표 6행 · 밝은 배경 열) · `SKILL.md` · `references/{spec,api,direction}.md`
의 개수(테마 21 · 배경 26 · 벡터 108). `assets/skins.js` 주석과 `starter-skins.json` 의
"테마 15종" 문구도 실제와 맞췄다. **설치본(`~/.claude/skills/gmotion`)까지 복사해
`diff -rq` 차이 0** 을 확인했다.

**검증.** `gm test` 291건 통과(문서 개수 대조·테마 이름 대조·대비 검사 포함) ·
`vitest run` 102건 통과(에디터의 번들 테마 대비 검사가 21종을 전부 훑는다) ·
여섯 테마를 같은 스펙으로 빌드해 씬별 스크린샷을 눈으로 확인했다(hero·cards·quote).
예제 스냅샷 기준값은 그대로다 — 기존 테마의 색·배경을 건드리지 않았다.
