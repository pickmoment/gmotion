...
| 플래그 | 효과 |
|---|---|
| `--present` | **발표용으로 출력** — 씬 단위 진행 + 발표자 창 (아래) |
...
| `--captions` | 화면 자막을 얹는다 (`--subs` 와 함께). 화면 맨 아래에 붙고, 보는 쪽에서 `C` 키나 플레이어의 `CC` 버튼으로 끌 수 있다 |
...
| `ART` 일러스트 | 20 | `<g class="gg-artP">` 로 부분이 나뉘어 있어 조각이 스태거로 등장한다 |
