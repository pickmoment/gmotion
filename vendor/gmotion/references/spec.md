...
트랜지션: `cut` `fade` `pushLeft` `pushRight` `pushUp` `zoomIn` `zoomOut` `wipe` `match` `curve`
...
녹음이 끝나 자막(SRT·VTT)이 있으면 화면을 목소리에 맞출 수 있다. 씬에 `say` 를
적고 `--subs` 로 빌드하면 씬의 시작·길이가 실측으로 바뀐다.

```jsonc
{ "pattern": "dataCounter",
  "say": "시작 자본은 백만 원이고 거래는 모두 열 번이라고 가정하겠습니다. …",
  "stats": [
    { "value": 100, "unit": "만 원", "label": "시작 자본",
      "say": "시작 자본은 백만 원이고" },        // 이 대사가 나올 때 이 지표가 들어온다
    { "value": 9,  "unit": "만 원", "label": "열 번째 거래 뒤",
      "say": "열 번째 거래에서 백만 원을 잃으면 남는 돈은 구만 원입니다." }
  ] }
```

지켜야 할 것이 셋 있다.

**`say` 는 자막의 연속 구간이어야 한다.** 중간 문장을 건너뛰고 이어 붙이면 정렬이
깨진다. 표현이 조금 달라지는 건(어미·조사·오탈자) 견디지만, 문장을 빼면 못 찾는다.

**항목에도 `say` 를 단다.** 씬 길이만 맞추면 요소가 처음 2초에 다 나오고 나머지
40초는 정지 화면이 된다. 항목마다 대사를 달면 그 대사에 맞춰 하나씩 들어온다.
항목 배열을 가진 패턴이면 어디서나 쓸 수 있다 — `items` `stats` `steps` `lines`
`nodes` `layers` `events` `stops` `sources` `targets` `orbits`, 그리고 차트의 `data.items`.

**좌우형은 쪽에 단다.** `splitCompare` 는 `left.say`/`right.say`, `beforeAfter` 는
`before.say`/`after.say` 다. 한쪽을 말하는 동안 다른 쪽은 아직 없다.

```jsonc
{ "pattern": "splitCompare",
  "say": "감당 가능한 손실은 반복 과정의 비용이 될 수 있습니다. …",
  "left":  { "label": "감당 가능한 손실",   "items": ["반복 과정의 비용"],
             "say": "감당 가능한 손실은 반복 과정의 비용이 될 수 있습니다." },
  "right": { "label": "감당 불가능한 손실", "items": ["반복 과정 자체를 없앤다"],
             "say": "감당 불가능한 손실은 반복 과정 자체를 없앱니다." } }
```

**첫 요소는 씬의 첫 대사에 붙인다.** 첫 요소의 `say` 를 씬 중반 대사에 걸면
그때까지 제목만 있는 빈 화면이 된다. 빌드가 이걸 경고한다 — 경고가 뜨면
대개 씬을 쪼개야 한다는 뜻이다.

앵커링할 요소가 없는 패턴이 넷 있다 — `heroReveal` `quote` `matchCut` `marquee`.
한 화면에 하나뿐이라 쪼갤 대상이 없다. 이 패턴으로 30초를 덮으면 30초가 정지다.
...
{ "pattern": "deviceShow",
  "title": "한 곳에서 답이 나온다",
  "frame": "browser",                    // browser window terminal phone tablet laptop card chat
