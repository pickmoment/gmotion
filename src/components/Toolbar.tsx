  ...
  return (
    <header className="toolbar" ref={host}>
      ...
      <div className="tb-group">
        <button type="button" onClick={() => void onNew()}>새로</button>
      ...
      <div className="tb-group menu-host">
        ...
        <button type="button" className={subsPath ? "on" : ""} onClick={() => setMenu(menu === "sync" ? null : "sync")}>
          자막·음성
        ...
        {menu === "sync" && (
          <div className="menu wide-menu">
            ...
            <p className="hint">
              순서는 자막이 먼저, 음성이 그다음이다. <code>--audio</code> 만 주면 소리는 실측인데 화면은 추정이라 어긋난다.
            </p>
            ...
            <div className="menu-row end">
              ...
              <button type="button" className="ghost" onClick={close}>닫기</button>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="tb-group menu-host">
        ...
        {menu === "export" && (
          <div className="menu">
            {EXPORTS.map((e) => (
              <button key={e.key} type="button" onClick={() => { onExport(e.key); close(); }}>
        ...
        <button type="button" onClick={onCheck} title="산출물 기계 검수 (gm check)">검수</button>
      </div>

      <div className="tb-group right">
        <button type="button" onClick={onDocs}>문서</button>
        <button type="button" onClick={onSkill}>스킬 설치</button>
      </div>
    </header>
  );
}
