...
**벡터 이름은 기억으로 쓰지 않는다.** 픽토그램 238종 + 벡터 세트 108종이다:

```bash
node <skill>/assets/gm.js icons 채팅       # 픽토그램 (한글 이름 지원)
node <skill>/assets/gm.js info decor       # 배경 26 · mark 15 · frame 19 · art 48
...
**카메라도 기본으로 움직인다** — 씬마다 아주 느린 카메라가 씬 전체 길이(hold 포함)를
덮어 정지 프레임을 없앤다. 배경은 그보다 덜 따라와 깊이가 생기고, 움직임이 있는
전환에는 짧은 잔상이 붙는다. 방향만 씬의 `cam` 으로 고른다(`gm info cam`) —
대개 기본값이 맞다. 전체를 세우려면 루트 `camera: false`.
...
**경로는 스펙에 적어 둔다.** 루트에
`"media": { "subs": "voice.srt", "audio": "voice.mp3", "captions": true }` 를 쓰면
다음부터 `gm build spec.json` 만으로 같은 결과가 나오고 앱도 자동으로 인식한다.
경로 기준은 스펙 파일이 있는 폴더다. 플래그를 함께 주면 플래그가 이긴다.
**자막·음성으로 스펙을 만들 때는 `media` 를 적는 것이 기본이다** — 상세는
`references/api.md` 의 자막 동기화 절.
...
assets/
  ...
  vectors.js       벡터 세트 108종 — 배경 26 · 강조 15 · 프레임 19 · 일러스트 48. 전부 코드로 그린다
